...
class UserSummaryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def generate(self, request):
        ...
        Body: {"language": "en" or "ar"}
        """
        user = request.user
        language = request.data.get('language', user.language_preference)
        ...
        try:
            ...
            summary_text, topics, preferences = AIService.generate_user_summary(
                user_messages=message_texts,
                language=language
            )
            
            # Count chats and messages
            chat_count = Chat.objects.filter(user=user, language=language).count()
            ...
            summary, created = UserSummary.objects.update_or_create(
                ...
                defaults={
                    ...
                    'ai_model_used': 'openai'  # Default, should be from AI service
                }
            )
